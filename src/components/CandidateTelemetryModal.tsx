...
export const CandidateTelemetryModal: React.FC<CandidateTelemetryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-4 bg-black/75 backdrop-blur-sm animate-fadeIn font-sans">
      <div className="w-full max-w-lg max-h-[92vh] overflow-y-auto bg-white dark:bg-[#0c0c0e] rounded-3xl border border-slate-200 dark:border-neutral-800 shadow-2xl p-5 md:p-6 text-slate-900 dark:text-neutral-100 relative">
        
        {/* Close button */}
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-neutral-800 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        {/* Candidate ID Card */}
        <div className="p-3 rounded-2xl bg-slate-50 dark:bg-[#121214] border border-slate-200 dark:border-neutral-800 mb-4 relative overflow-hidden">
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 rounded-2xl bg-white dark:bg-neutral-800 border border-slate-200 dark:border-neutral-700 flex items-center justify-center text-3xl shrink-0 shadow-sm">
              {profile.avatarEmoji}
            </div>
          ...
          <div className="mt-3 pt-2.5 border-t border-slate-200 dark:border-neutral-800 flex items-center justify-between text-xs">
            <span className="text-slate-500 dark:text-neutral-400 font-medium">FASE ELECTORAL:</span>
        ...
        <div className="flex items-center justify-between text-[11px] font-bold text-slate-600 dark:text-neutral-400 uppercase tracking-wider mb-2">
        ...
        <div className="space-y-2.5">
          ...
          <div className="bg-slate-50 dark:bg-[#121214] p-3 rounded-2xl border border-slate-200 dark:border-neutral-800">
            <div className="flex items-center justify-between text-xs mb-1.5">
              ...
              <span className={`font-black text-base font-mono ${
                ...
                stats.polling >= 14 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-700 dark:text-neutral-300'
              }`}>
          ...
          <div className={`p-3 rounded-2xl border transition-all ${
            stats.jneTachaRisk >= 65 ? 'bg-red-50 dark:bg-red-950/60 border-red-300 dark:border-red-600 animate-pulse' : 'bg-slate-50 dark:bg-[#121214] border border-slate-200 dark:border-neutral-800'
          }`}>
            <div className="flex items-center justify-between text-xs mb-1.5">
              <span className="text-slate-700 dark:text-neutral-300 font-semibold flex items-center gap-1.5">
        ...
        {/* Modal Action Footer */}
        <div className="mt-5 pt-3 border-t border-slate-200 dark:border-neutral-800">
          <button
            type="button"
            onClick={onClose}
