...
export const WeeklyPollModal: React.FC<WeeklyPollModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-4 bg-slate-950/70 backdrop-blur-md animate-fadeIn font-sans">
      
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 md:p-7 shadow-2xl relative text-slate-900 dark:text-slate-100">
        ...
        <div className="flex items-center justify-between pb-3.5 mb-4 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="text-right hidden sm:block">
            <div className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">FICHA TÉCNICA</div>
            <div className="text-xs text-slate-600 dark:text-slate-300 font-bold">1,200 casos • E: +/-2.8%</div>
          </div>
        </div>

        {/* Player Status Callout */}
        <div className="bg-slate-50 dark:bg-slate-950/80 rounded-2xl p-4 border border-slate-200 dark:border-slate-800 mb-5 flex items-center justify-between gap-4">
          <div>
          ...
          <div className="text-right shrink-0">
            <div className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">VARIACIÓN SEMANAL</div>
            ...
            return (
              <div key={cand.id} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className={`flex items-center gap-1.5 truncate ${isUser ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-slate-700 dark:text-slate-300 font-medium'}`}>
                    <span className="font-bold text-[11px] text-slate-400 w-5">#{idx + 1}</span>
                    <span className="truncate">{cand.name}</span>
                    <span className="text-[10px] text-slate-400 hidden sm:inline">({cand.partyShort})</span>
                    {isUser && (
                  ...
                  <span className={`font-bold shrink-0 ${isUser ? 'text-blue-600 dark:text-blue-400 text-sm font-black' : 'text-slate-600 dark:text-slate-400 text-xs'}`}>
        ...
        {/* Analysis Commentary */}
        <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 leading-relaxed mb-5">
        ...
        <button
          type="button"
          onClick={onContinue}
          className="w-full py-3 px-5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-md active:scale-98 transition-all cursor-pointer"
        >
